...
- **Core (breaking): replaced the overloaded `isTty` boolean with a `Stdio` primitive.** `RunContext.isTty`, `RunOptions.isTty`, `DispatchOptions.isTty`, and `CliEvent.isTty` are removed. Handlers now read `ctx.stdio` — per-stream `fstat` classification (`stdin`/`stdout`/`stderr` → `{ kind, isTTY }` where `kind` is `tty`/`pipe`/`file`/`socket`/`char`/`closed`), plus `color` (stdout color support, delegated to the runtime's `tty.WriteStream#getColorDepth` — parses `FORCE_COLOR` by value, honors `NO_COLOR`/`COLORTERM`/`TERM`/`CI`), `width`, and `interactive` (stdin && stdout are terminals — feasibility, not "a human is present"). Tests/adapters inject `RunOptions.streams` / `DispatchOptions.streams` (`StreamOverrides`); lifecycle events and the telemetry wire schema carry `streams: { stdin, stdout, stderr }` instead of `isTty`. New public exports: `nonInteractiveStdio`, `streamKinds`, and types `Stdio`, `StreamView`, `StreamKind`, `StreamKinds`, `ColorSupport`, `ColorLevel`, `StreamOverrides`. Hard cutover, no compatibility shim. See [docs/stdio-primitive-plan.md](./docs/stdio-primitive-plan.md).
  ...
  - Terminal output now writes synchronously (`fs.writeSync`): `… | head` exits cleanly instead of crashing with an unhandled `EPIPE`, and large output on a nonzero exit is no longer truncated by an eager `process.exit`. Streaming (`--format jsonl`) chunk output is awaited per chunk (the runner no longer fire-and-forgets `Bun.stdout.write`), so chunks land in yield order even to a file sink and flush before a nonzero exit; the streaming broken-pipe case (`… | head`) also exits cleanly. Verified on macOS arm64 (Bun 1.3.9) and Linux x64 (Bun 1.3.14).

- **Core (breaking): unified terminal-only flags behind the public `TerminalHandler` contract.** `--version`/`--help`/`--schema` are no longer hardcoded by flag name in the runner and result lanes; they are registered through the same public `terminalHandlers` API extensions use (`--llms`, etc.). The contract gains optional `commandAware` (run before vs after command selection), `event` (declarative lifecycle event), and `matches` (e.g. the help no-command fallback); `TerminalHandlerInput` gains `selected` (the new public `TerminalCommandInfo` projection — `{ path, contract }`, never the internal entry), `format`, and an EPIPE-safe `io`. `dispatch`/`parseInvocation` reject run-only flags generically (no hardcoded names); duplicate terminal flag keys now throw at CLI creation. Terminal-handler lifecycle events now carry `surface: { kind: 'terminal', name }` (the dedicated `version`/`help`/`schema` surface kinds are removed). New public exports: `buildHelpModel`, and types `TerminalCommandInfo`, `TerminalIo`, `TerminalHandlerInput`. Additive for handler authors (only `flagKey`+`handle` remain required).

- **Core (breaking): `--help` and `--version` are now first-class, default-on `defineCli` fields, and the `help()`/`version()` control factories are removed from the public API.** Help renders by default — fixing the gap where a CLI without `help()` printed nothing for bare invocation, an unknown command, or `--schema` with no command selected. `help: false` opts out; `help: { renderer }` customizes it (the standalone `helpRenderer` field is folded into this single knob). `--version` registers automatically whenever a `version` string is set. Both are still installed internally through the public `TerminalHandler` contract, so a user-registered handler for the same flag wins. `--schema` (`reflectionControls()`) stays opt-in; `outputControls`, `reflectionControls`, and the `HelpControlOptions` type remain exported. Generated CLIs no longer emit `help()`/`version()` in their `extensions` array.

- **Core (breaking): removed the `vars` schema declaration from `defineCli`.** The CLI-level `vars` Zod schema — which only seeded defaults by parsing an empty object and never typed `ctx.var` — is gone, along with `DefineCliOptions.vars`, `CliInstance.vars`, and `ParsedInvocation.input.vars`. The `ctx.var` / `ctx.set` run-context scratch bag is unchanged: it now simply starts empty (`{}`), and middleware/hooks seed it imperatively via `ctx.set(key, value)` — the way the canonical "middleware owns a resource" pattern already worked.
