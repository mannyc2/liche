...
| `outputControls`, `reflectionControls` | Opt-in Core controls that install standard runtime globals such as `--json`, `--format`, and `--schema`. (`--help` and `--version` are first-class `defineCli` fields — help is on by default; `--version` registers when a version string is set — registered internally through the public `TerminalHandler` contract, not exported as controls.) |
| `buildHelpModel` | Builds the serializable `HelpModel` for a selected command (`buildHelpModel(name, state, selectCommand(state, path), rest)`); pair with `defaultHelpRenderer` so a command-aware terminal handler can reuse the framework's help rendering. |
| `defaultHelpRenderer` | Default renderer for the serializable help model; a custom `defineCli({ help: { renderer } })` can wrap it. |
