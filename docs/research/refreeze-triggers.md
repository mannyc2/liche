...
| Config primitive re-freeze | Generated remote base URLs needed typed durable config, explicit option binding, strict validation, and provenance. | Added `createConfig`, `ctx.config`, `ctx.sources`, config scopes, discovery, and explicit option-to-config bindings. | General config stays data-only and opt-in; auth/session state stays outside the durable config ladder. |
