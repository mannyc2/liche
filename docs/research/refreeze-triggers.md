...
| Auth/session 3D-A re-freeze | Generated remote-operation CLIs needed shared credential/context resolution, redaction, and invocation posture. | Added `secret`, `resolveAuth`, `resolveContext`, `applyAuth`, auth metadata types, `RunContext.invocation`, and structured auth details. A later minimization pass internalized `authMetaFromCredential` because generated/package consumers did not need it at the root. | Auth stays in core only where it changes command safety/security semantics. Application auth commands remain generated/application-level. |
