...
| SKILLS-ADD-001 | skills add agent | When the `skills` helper built-in is enabled, `skills add --agent <agent>` writes to the agent's skill directory; `--no-global` chooses the project location; packaged skill content is used when `CreateOptions.skill` is set. | `parity.test.ts`, `skill-markdown.test.ts` | Hardcoding `~/.claude/skills/`, ignoring packaged skill content, or exposing helper commands without opt-in. |
| GEN-001 | Typegen built-in | When the `gen` helper built-in is enabled, `<cli> gen` writes `lili.generated.ts` containing a `declare module` block that augments `Cli.Commands` with each command path and its arg/option schema. | `parity.test.ts` | Generating an empty file, omitting paths, or exposing helper commands without opt-in. |
