...
| Remote transport and server conformance | V1 blocker for remote-backed Product examples. | Core `serializeHttpOperationRequest` and `callHttpOperation` are implemented. Remaining work is the Product config/base URL contract from `docs/config-primitive.md`, generated remote wiring through those APIs, fixture-backed conformance, and report redaction checks. | Core serializer/transport tests pass now; generated remote command tests, local fixture-server conformance tests, and report redaction checks must pass before V1. |
| Generated surface completion | V1 blocker for any surface named in public docs. | Finish catalog-derived command manifest, MCP command tools, Agent Skill/LLM surfaces, docs/reference markdown, and config JSON Schema when general config or bindings are declared. Keep OpenAPI-derived downstream adapters separate. | `generate --check` tracks every emitted surface by id; stale CLI/OpenAPI/MCP/docs/agent/config artifacts fail with targeted drift. |
| Sessions and OAuth device login | V1 blocker for authenticated remote-operation CLIs. | Finish file sessions, profiles, context selection, generated `whoami`/`switch`/`login`/`logout`, OAuth device login without refresh tokens, identity probes, env-token CI/agent mode, and no implicit login for normal commands. | Session/OAuth examples run in temp stores; normal, CI, agent, and MCP invocations never start interactive login implicitly; generated auth commands are catalog capabilities with auth effects. |
...
Product-generated config schema needs the first-class config primitive before telemetry config can be documented publicly. If `lili.generated.config.schema.json` is the full CLI config schema, it must include declared Product config, bindings, and explicitly reserved framework namespaces such as `lili.telemetry`; unknown product keys still fail under the strict config contract. If the generated artifact remains binding-only, it must be renamed or documented as a binding config schema so it does not masquerade as the full runtime config contract.
