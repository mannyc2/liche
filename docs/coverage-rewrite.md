...
| BUILD-006 | Locality is required and bound. | `docs/build-system.md` | Lint operation without locality, with a default outside modes, and with local/remote modes missing their execution binding. | Requirement fixture | Local/remote behavior inferred, omitted, or declared without an implementation binding. |
...
| BUILD-020 | Default generated vocabulary is replaceable. | `docs/build-system.md` | Normalize a contract with an explicit vocabulary object and verify defaults are absent. | Requirement fixture | Defaults become mandatory instead of a convenience preset. |
...
| RELEASE-003 | Manifest records runtime env/config expectations. | `docs/distribution.md` | Remote config fixture includes env expectations. | Contract IR | Binary runtime contract discoverable only by failing at runtime. |
