...
| BUILD-026 | Product schema uses static class authoring but digests normalized plain data. | `docs/product-schema.md` | Build equivalent `Product.create()` schemas through separate class instances and compare catalog digests. | Canonical catalog | Digest depends on class identity, private fields, or construction side effects. |
