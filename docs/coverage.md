...
| DISPATCH-001 | Public invocation | `dispatch(cli, argv, options)` executes through the shared command pipeline and returns `Result` without writing stdout/stderr or calling exit. | planned: `invocation-api.test.ts`, temp-consumer proof | First-party adapters compose `getCliState`/`selectCommand`/`execute`, or tests mock process I/O for result assertions. |
| PARSE-001 | Public invocation | `parse(cli, argv, options)` selects the command and resolves globals/input/provenance without running the handler. | planned: `invocation-api.test.ts` | Parse-only users must execute handlers or import parser/executor internals. |
...
| ARG-CODEC-002 | Custom argument codecs | `arg.fromString()` decodes string inputs into handler runtime values while command contracts project the declared input schema. | planned: `arg-codecs.test.ts`, `contract.test.ts` | Custom decoded values force userland parser code or leak runtime-only types into JSON Schema/MCP. |
| ARG-CODEC-003 | Async argument codecs | Command input validation awaits async codec decoders/refinements and normalizes failures into the standard error envelope. | planned: `arg-codecs.test.ts` | Async decoders throw `ZodAsyncError`, run after the handler, or bypass structured validation errors. |
...
| ERROR-003 | Source-aware validation errors | Validation errors preserve field path, issue metadata, and value source such as argv flag, positional index, env, provider/config path, fetch query/body, MCP, or programmatic input. | planned: `arg-codecs.test.ts`, `contract.test.ts`, fetch/MCP focused tests | Human and machine errors force users to guess where a bad value originated. |
...
| MCP-HIDDEN-001 | MCP hidden commands | Commands with `agent: false`, including local extension helper commands, are omitted from `tools/list` and cannot be invoked through `tools/call` by guessing the tool name. | `parity.test.ts` | Hiding tools from discovery but still allowing direct MCP invocation. |
...
| AGENT-FLIP-001 | `--json` flips agent | Explicit `--json`/`--format` from installed output controls sets `c.agent` to `true` even on a TTY. | `parity.test.ts` | Leaving `c.agent` tied to the raw TTY check or accepting output globals without opt-in. |
...
| RUN-001 / DISPATCH-001 / PARSE-001 | Public invocation APIs replace internal executor composition. | core-run-and-arg-codecs-plan.md | `invocation-api.test.ts` plus a temp-consumer package-root import proof. | Public `Result` and parse fixtures | Adapters keep importing `CliState`, `SelectedCommand`, or `execute` to run commands. |
| ARG-CODEC-001 / ARG-CODEC-002 / ARG-CODEC-003 | Strict Zod-backed CLI argument codecs. | core-run-and-arg-codecs-plan.md | `arg-codecs.test.ts`, command schema projection tests, and property tests for numeric grammar. | Zod codec docs + JSON Schema input projection | Broad coercion remains the documented path or runtime-only outputs leak into contracts. |
| ERROR-003 | Source-aware validation errors. | core-run-and-arg-codecs-plan.md | Validation failures from argv/env/provider/fetch/MCP preserve source metadata in machine and human output. | Structured Zod issues + Core source provenance | Error rendering scrapes messages or drops source context. |
