...
| PARSE-001 | Public invocation | `parseInvocation(cli, argv, options)` selects the command and resolves globals/input/provenance without running the handler. | planned: `invocation-api.test.ts` | Parse-only users must execute handlers or import parser/executor internals. |
...
| MCP-HIDDEN-001 | MCP hidden commands | Commands with `interactive: true` (commands that need a human prompt/TTY flow) are omitted from `tools/list` and cannot be invoked through `tools/call` by guessing the tool name. | `parity.test.ts` | Hiding tools from discovery but still allowing direct MCP invocation. |
