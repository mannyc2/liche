...
export async function authWhoami(input: AuthRuntimeInput): Promise<AuthStatus> {
  ...
  const stored =
    !input.global?.noSession && storedName
...
export async function probeIdentity(
  ...
  // valueAt walks a product-configured dotted path into untyped endpoint JSON, so it returns
  // `unknown`: a mis-pointed or shape-drifted path can resolve to an object/array that String()s
  // to "[object Object]". Require a scalar before coercing, so a bad path fails loudly here instead
  // of persisting a corrupt account id/label into the session store and surfacing it in `whoami`.
  const id = valueAt(body, input.identity.subject)
  if ((typeof id !== 'string' && typeof id !== 'number') || id === '') {
    throw authInvalid({ providerId: input.credential.providerId })
  }
  const rawLabel = input.identity.label ? valueAt(body, input.identity.label) : undefined
  const label = typeof rawLabel === 'string' || typeof rawLabel === 'number' ? String(rawLabel) : undefined
  return {
    id: String(id),
