...
function authError(
  ...
  return { code, details, exitCode: 1, message, ...recovery }
}
...
export function authMissing(input: {
  ...
  return authError('AUTH_MISSING', `Authentication required${detail}.`, {
...
export function authCiTokenMissing(input: { providerId: string; envVars: string[] }): CommandError {
...
export function authContextRequired(input: {
  ...
  const remedies = input.contexts.flatMap((c) => [
    ...(c.flag ? [`--${c.flag}`] : []),
    ...(c.envVar ? [c.envVar] : []),
  ])
...
export function authInvalid(input: { providerId: string; status?: number | undefined }): CommandError {
...
export function authInteractiveRequired(input: { providerId: string; loginCommand?: string | undefined }): CommandError {
...
export function authSessionCorrupt(input: { providerId?: string; profile?: string } = {}): CommandError {
  return authError('AUTH_SESSION_CORRUPT', 'Stored auth session is corrupt.', input)
}

export function authSessionLocked(input: { providerId?: string; profile?: string }): CommandError {
