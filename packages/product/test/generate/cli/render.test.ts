...
describe('generateCli — command tree shape', () => {
  ...
  test('root defineCli carries product id+version and explicit generated controls', () => {
    ...
    expect(source).toContain(`outputControls({ json: true, fullOutput: true, filterOutput: true, tokenCount: true, tokenLimit: true, tokenOffset: true })`)
    expect(source).toContain(`reflectionControls({ schema: true })`)
    expect(source).toContain(`llms()`)
  })
})
...
describe('generateCli — run bodies by execution mode', () => {
  ...
  test('remote-http command with product remote config calls core HTTP transport', () => {
    ...
    expect(source).toContain(`extensions: [help(), version(), outputControls({ json: true, fullOutput: true, filterOutput: true, tokenCount: true, tokenLimit: true, tokenOffset: true }), reflectionControls({ schema: true }), llms(), configExtension({`)
