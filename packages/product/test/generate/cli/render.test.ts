...
describe('generateCli — run bodies by execution mode', () => {
  ...
  test('remote-http command with product remote config calls core HTTP transport', () => {
    ...
    expect(source).toContain(`extensions: [help(), version(), outputControls({ json: true, fullOutput: true, filterOutput: true }), reflectionControls({ schema: true }), llms(), configExtension({`)
