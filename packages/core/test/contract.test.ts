...
describe('contract: args, flags, env, middleware', () => {
  ...
  test('option source provenance distinguishes argv, provider, and schema default', async () => {
    ...
    expect(
      parseJsonData((await runCli(cli, ['run', '--region', 'sfo', '--json'], { env: { APP_REGION: 'dfw' } })).stdout),
    ).toEqual({ region: 'sfo', source: { kind: 'argv' } })
    expect(parseJsonData((await runCli(cli, ['run', '--json'], { env: { APP_REGION: 'dfw' } })).stdout)).toEqual({
      region: 'dfw',
      source: { kind: 'provider', provider: 'env', path: 'APP_REGION', source: { kind: 'env', name: 'APP_REGION' } },
    })
    expect(parseJsonData((await runCli(cli, ['run', '--json'], { env: {} })).stdout)).toEqual({
      region: 'iad',
      source: { kind: 'default' },
    })
  })
...
describe('contract: fetch and schema', () => {
  ...
  test('fetch dispatch merges JSON body options and normalizes not found and validation errors', async () => {
    ...
    expect(await response.json()).toEqual({
      ok: true,
      data: { id: 7, options: { active: false, limit: 5 } },
      error: null,
    })
  ...
  test('fetch exposes MCP endpoint, HEAD behavior, malformed body rejection, and empty body defaults', async () => {
    const cli = testCli('api', { version: '3.0.0', extensions: [mcpServer()] }, [
      testCommand('echo', {
...
describe('contract: mcp, completions, and token behavior', () => {
  test('mcp initialize, tools/list, tools/call, and unknown method use JSON-RPC envelopes', async () => {
    const cli = testCli('app', { version: '1.2.3' }, [
      testCommand('echo', {
    ...
    await expect(
      Mcp.mcpMessage('app', state, {
        jsonrpc: '2.0',
        id: 3,
        method: 'tools/call',
        params: { name: 'echo', arguments: { args: { message: 'hi' } } },
      }),
  ...
  test('completions include commands and aliases without duplicates', () => {
    const cli = testCli('app', [
      testCommand('inspect', { aliases: ['i'], run: () => ({ ok: true }) }),
      testCommand('install', { run: () => ({ ok: true }) }),
    ])
    const state = (cli as InternalCli)[stateSymbol]
  ...
  test('run handles version, envelope output, filters, token limits, and CTA metadata', async () => {
    const cli = testCli('app', { version: '2.0.0' }, [
      testCommand('deploy', {
        run: ({ ok }) =>
          ok(
