...
describe('parity: deprecated option metadata', () => {
  ...
  test('deprecation warning fires on TTY only', async () => {
    ...
    const piped = await runCli(cli, ['build', '--legacy', '--json'], {
      streams: { stdin: 'pipe', stdout: 'pipe', stderr: 'pipe' },
    })
    expect(piped.stderr).toBe('')

    const tty = await runCli(cli, ['build', '--legacy', '--json'], {
...
describe('parity: --llms shape', () => {
  test('--llms with --format json returns the liche.v1 envelope', async () => {
    const cli = testCli('app', { description: 'app cli', extensions: [skillsRuntime()], version: '1.0.0' }, [
      testCommand('publish', {
