...
export type ParseWarning = { kind: 'deprecated-option'; flag: string; option: string }
...
export type MiddlewareHandler = (context: MiddlewareContext, next: () => Promise<void>) => Awaitable<unknown>
...
> = {
  ...
  sources?:
    ...
    | undefined
  // run may return command output, an AsyncGenerator (streaming), a Promise, or void — all `unknown`.
  run?:
    | ((context: RunContext<InferSchema<A>, InferSchema<O>, InferSchema<E>, Record<string, unknown>>) => unknown)
    | undefined
  summary?: string | undefined
...
> = Omit<CommandDefinition<A, E, O, Out>, 'alias' | 'aliases' | 'args' | 'env' | 'options' | 'run'> & {
  ...
  // run may return command output, an AsyncGenerator (streaming), a Promise, or void — all `unknown`.
  run?: ((context: DeclarativeCommandRunContext<A, E, O>) => unknown) | undefined
  summary?: string | undefined
}
